...
begin
      ...
      if (rising_edge(clk)) then
        if (rupdate='0') then
          ...
          --    rack  <= '1';
              ...
              rack  <= '1';
            -- RW registers
            elsif (reg=C_REG_TP_CONFIG_A) then
              rdata <= test_pattern_config_a;
              rack  <= '1';
  ...
  begin
    if (rst = '1') then
      test_pattern_config_a   <= x"00000000";
      test_pattern_config_b   <= x"00000000";
...
--        commands   <= x"00000000";
        if (wupdate='0') then
          ...
          if (scope=C_SCOPE) then
            if (reg=C_REG_TP_CONFIG_A) then
              test_pattern_config_a   <= wdata;
              wack         <= '1';
            elsif (reg=C_REG_TP_CONFIG_B) then
